		...
		if not product_obj:
			...
			if product_obj:
				...
				self.env['connector.snippet'].create_odoo_connector_mapping('connector.product.mapping',
											...
											instance_id,
											created_by=channel)
				if 'ecomm_combination_id' not in ctx:
					template_id = product_obj.product_tmpl_id.id
					self.env['connector.snippet'].create_odoo_connector_mapping('connector.template.mapping', ecomm_id,
					template_id, instance_id,
					is_variants=False, created_by=channel)
			else:
